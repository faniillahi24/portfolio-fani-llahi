...
import { useState, useEffect } from 'react'; // Mengimpor hooks yang diperlukan untuk Typewriter

// =======================================================================================================
// === KOMPONEN TYPEWRITER (DIGABUNGKAN UNTUK MENGHILANGKAN ERROR RESOLVE) ===
// =======================================================================================================
...
    const [index, setIndex] = useState(0); // Indeks role saat ini
    const [subIndex, setSubIndex] = useState(0); // Indeks karakter saat ini
    const [reverse, setReverse] = useState(false); // Mode mengetik atau menghapus
    const [blink, setBlink] = useState(true); // Status blink kursor

    // Efek untuk Typewriter Logic
    useEffect(() => {
        if (index === roles.length) {
            setIndex(0); // Reset ke role pertama setelah selesai loop
        }

        if (subIndex === roles[index].length + 1 && !reverse) {
            // Tunggu setelah selesai mengetik, lalu mulai hapus
...
        }, reverse ? speed / 2 : speed); // Kecepatan hapus dua kali lebih cepat

        return () => clearTimeout(timeout);
    }, [subIndex, index, reverse, roles, speed, delay]);

    // Efek untuk Cursor Blinking
    useEffect(() => {
...
            <span className={`border-r-2 border-sky-400 font-normal opacity-100 ${blink ? 'opacity-100' : 'opacity-0'} transition-opacity duration-100`} aria-hidden="true">
                &nbsp;
            </span>
        </>
    );
...
    return (
        <section id="beranda" className="relative min-h-screen flex items-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white pt-16 overflow-hidden">
...
                <div className="absolute top-1/2 right-1/3 w-64 h-64 bg-violet-500/5 rounded-full blur-3xl animate-pulse" style={{animationDelay: '2s'}}></div>
            </div>

            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
                <div className="grid md:grid-cols-2 gap-x-16 lg:gap-x-32 items-start">
                    
...
                            <p className="text-sm text-sky-300 font-medium tracking-wide">Kode yang Indah, lahir dari ketekunan.</p>
                        </div>
...
                                    className="group relative w-12 h-12 flex items-center justify-center bg-slate-800/80 backdrop-blur-sm rounded-xl text-slate-300 transition-all duration-300 hover:bg-gradient-to-br hover:from-sky-600 hover:to-indigo-600 hover:text-white hover:shadow-lg hover:shadow-sky-500/50 hover:-translate-y-1"
                                >
...
                                    alt="Profile"
                                    className="relative rounded-2xl w-72 sm:w-80 lg:w-96 h-auto object-cover border-4 border-slate-900 shadow-2xl"
                                />
                                
